...
import com.clexi.hio.dialog.AccountsDialog;
import com.clexi.hio.helper.Broadcaster;
...
public class AccessibilityManager extends AccessibilityService
{
    ...
    public void onAccessibilityEvent(AccessibilityEvent event)
    {
        ...
        else if (event.getEventType() == AccessibilityEvent.TYPE_WINDOW_STATE_CHANGED)
        {
            ...
            if (window == null || window.getPackageName() == null)
            {
            ...
            if (window.getPackageName().equals("com.clexi.hio"))
            {
                // Give up, it's me :)
                Log.d(TAG, "PackageName of event refers to our app.");
            }
            else
            {
            ...
            public void onReceive(Context context, Intent intent)
            {
                if (intent.getAction().equals(Consts.ACTION_LOGIN_REQUEST))
                {
                    ...
                    if (matchingLogins == null || matchingLogins.size() == 0)
                    {
                        ...
                        if (!com.clexi.hio.helper.Utils.isMyServiceRunning(getApplicationContext(), AccountsDialog.class))
                        {
                            /*Start the Service*/
